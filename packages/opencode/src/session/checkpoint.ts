...
async function ensureCheckpointTemplate(checkpointFile: string): Promise<void> {
  const exists = await fs.stat(checkpointFile).then(() => true).catch(() => false)
  if (!exists) {
    await fs.mkdir(path.dirname(checkpointFile), { recursive: true })
    await fs.writeFile(checkpointFile, CHECKPOINT_TEMPLATE, "utf-8")
  }
}

async function ensureMemoryTemplate(memoryFile: string): Promise<void> {
  const exists = await fs.stat(memoryFile).then(() => true).catch(() => false)
  if (!exists) {
...
async function ensureNotesTemplate(notesFile: string): Promise<void> {
  const exists = await fs.stat(notesFile).then(() => true).catch(() => false)
  if (!exists) {
    await fs.mkdir(path.dirname(notesFile), { recursive: true })
    await fs.writeFile(notesFile, NOTES_TEMPLATE, "utf-8")
  }
}
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const tryStartCheckpointWriter: (
      ...
      const checkpointExists = yield* Effect.promise(() => fs.stat(checkpointFile).then(() => true).catch(() => false))
      const memoryExists = yield* Effect.promise(() => fs.stat(memoryFile).then(() => true).catch(() => false))
    ...
    const hasCheckpoint = Effect.fn("SessionCheckpoint.hasCheckpoint")(function* (sessionID: SessionID) {
      return yield* Effect.promise(() => fs.stat(checkpointPath(sessionID)).then(() => true).catch(() => false))
    })
    ...
    const renderIndex = Effect.fn("SessionCheckpoint.renderIndex")(function* (sessionID: SessionID) {
      ...
      const exists = yield* Effect.promise(() => fs.stat(snapFile).then(() => true).catch(() => false))
